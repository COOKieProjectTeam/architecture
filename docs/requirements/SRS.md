...
Канон **модели данных рецептов (DR-REC)** на текущем этапе задаётся только **текстом в этом параграфе** и перекрёстной [трассировкой](./traceability.md). Полноформатная предметная модель, диаграммы UML, схема БД в графической нотации и визуальный дизайн — **последующие артефакты** под `architecture/` и FRS; синхронизация пакета документации с репозиторием см. [SYNC](../SYNC.md).
